...
class UserWarnCog(ConfiguredCog):
    ...
    async def warn(self, ctx: commands.Context, action: str, *user_name_list: str):
        ...
        :param action:  The string action to execute. Should correlate to an action in
                        the `WarnAction` enumeration.
        :param user_name_list: List[str]    A list of strings, denoting either a user's nickname, or their
    ...
    def _remove_outdated_warnings(self, target_member: Member):
        ...
        :param target_member:   The member to look for when removing outdated warnings.
        """
    ...
    def _warn_member(target_member: Member) -> int:
        ...
        # Find the user in the db so that we can attach a warning to it (should add a user if none found)
    ...
    def _remove_warning(target_member: Member, action: str) -> int:
        ...
        :param target_member:   The member to add a warning to.
        :param action:          The action to perform. MUST be either `WarnAction.RESOLVE` or `WarnAction.UNDO`.
    ...
    def _find_discord_member(self, user_query: str) -> list:
        ...
        :param user_query:  The query to search for, either an integer discord ID or a member nickname.

        :return:    A list of `discord.Member` instances, each one relating to a member that matched the query, or an
    ...
    def _multi_member_found_message(user_search_query: str, member_matches: list) -> str:
        ...
        :param user_search_query:   The search query that was used to find all the members listed.
        :param member_matches:      A list of `discord.Member` instances, each relating to a member that matched the
                                    search query.

        :return:    A user-friendly error message reporting all the matches found with the given error message.
        """
