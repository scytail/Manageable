...
class AirlockCog(ConfiguredCog):
    ...
    async def on_message(self, message: Message):
        """Watches for any messages sent in the airlock channel, and deletes them after five seconds.
           ...
           :param message:  The message that was sent in a place that the bot can see
           """
        airlock_channel_name = ConfiguredCog.config['content']['airlock_channel']
...
class HelpCog(ConfiguredCog):
    ...
    @commands.command()
    async def help(self, ctx: commands.context, command: Union[str, None] = None):
    ...
    def _build_help_summary(self, help_dict: dict) -> list:
        ...
        :return:    A list of `discord.Embed` objects that will be used as pages when browsing the help command.
        """
    ...
    def _build_help_detail(self, help_dict: dict, command_name: str) -> list:
        ...
        :param command_name:    The command keyword to search the help dictionary for.

        :return:    A list of `discord.Embed` objects, where each embed is a page to display that contains help
                    information.
        """
    ...
    def _get_enabled_commands(self, help_dict: dict) -> dict:
        """Compile a dictionary of all the valid commands from all the enabled cogs, where the key is the command, and
        the value is the description.

        :param help_dict:   The data dictionary that has the help information.

        :return:    A dictionary where the key is the command and the value is a dict with a 'description' and 'details'
        """
    ...
    def _get_check_method(self, message: Message, allow_decrease: bool, allow_increase: bool) -> callable([..., bool]):
        ...
        :param message:         The message instance that we are watching reacts from.
        ...
        :return:    A method that takes some number of arguments and returns a boolean on whether you can move to the
                    page desired.
        """

        def _check(reaction: discord.Reaction, user: discord.ClientUser) -> bool:
            """Checks to see if the reaction can trigger the help pagination event.
